...
class GroupCreatorBot:
    ...
    async def run_group_creation_worker_api(self, user_id: int, account_name: str):
        ...
        user_client = TelegramClient(str(session_path), API_ID, API_HASH, device_model=selected_device.get('device_model'), system_version=selected_device.get('system_version'), app_version=selected_device.get('app_version'))
            ...
            if text == Config.METHOD_API:
                ...
                try:
                    async with self.bot.conversation(user_id, timeout=300) as conv:
                        ...
                        await conv.send_message("آیا این حساب رمز تایید دو مرحله‌ای دارد؟ (بله / خیر)")
                        pass_prompt = await conv.get_response()
                        password = None
                        if pass_prompt.text.lower() in ['بله', 'yes']:
                            await conv.send_message("لطفا رمز تایید دو مرحله‌ای را وارد کنید:")
                        ...
                        success = await loop.run_in_executor(None, selenium_client.login, phone, code, password)

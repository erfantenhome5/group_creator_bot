...
class Config:
    ...
    METHOD_SELENIUM = "🛡️ Selenium (امن)"
...
API_ID = int(API_ID)
API_SESSIONS_DIR = Path("api_sessions")
SELENIUM_SESSIONS_DIR = Path("selenium_sessions_api_backend") # Renamed to clarify
API_SESSIONS_DIR.mkdir(exist_ok=True)
...
class GroupCreatorBot:
    ...
    def get_all_accounts(self) -> Dict[str, List[str]]:
        ...
        for d in API_SESSIONS_DIR.iterdir():
    ...
    def get_account_type(self, account_name: str) -> Optional[str]:
        if (API_SESSIONS_DIR / account_name).exists(): return 'api'
        if (SELENIUM_SESSIONS_DIR / account_name).exists(): return 'selenium'
        return None

    def _get_session_path(self, account_name: str, acc_type: str) -> Path:
        base_dir = API_SESSIONS_DIR if acc_type == 'api' else SELENIUM_SESSIONS_DIR
        return base_dir / account_name / f"{account_name}.session"
    ...
    def _delete_account(self, account_name: str) -> bool:
        ...
        dir_to_delete = API_SESSIONS_DIR / account_name if acc_type == 'api' else SELENIUM_SESSIONS_DIR / account_name
        try:
    ...
    # --- Worker Task ---
    async def run_group_creation_worker(self, user_id: int, account_name: str, acc_type: str):
        worker_key = f"{user_id}:{account_name}"
        session_path = self._get_session_path(account_name, acc_type)
        ...
        try:
            async with self.api_semaphore:
                ...
                current_counter = self._read_counter(account_name, acc_type)
    ...
    async def _handle_login_flow(self, event, text):
        ...
        if state == 'awaiting_name':
            ...
            phone = text.strip()
            selected_device = random.choice(Config.DEVICE_PROFILES)
            client = TelegramClient(StringSession(), API_ID, API_HASH, device_model=selected_device.get('device_model'), system_version=selected_device.get('system_version'), app_version=selected_device.get('app_version'))
            async with self.sessions_lock:
    ...
    async def _finalize_login(self, event):
        ...
        async with self.sessions_lock:
            ...
            acc_type = self.user_sessions[user_id]['acc_type']
    ...
    async def _message_router(self, event: events.NewMessage.Event):
        ...
        if state == 'awaiting_add_method':
            ...
            if text == Config.METHOD_API:
                acc_type = 'api'
            elif text == Config.METHOD_SELENIUM:
                acc_type = 'selenium'
            
            if acc_type:
                async with self.sessions_lock:
                    self.user_sessions[user_id] = {'state': 'adding_account', 'sub_state': 'awaiting_name', 'acc_type': acc_type}
